...
class WhatsAppDriver extends HttpDriver
{
    ...
    const API_URL = 'https://api.gupshup.io';
    ...
    protected $endpoint = '/sm/api/v1/msg';
    ...
    public function getUser(IncomingMessage $matchingMessage): User
    {
        ...
        if ($response->getStatusCode() !== 200) {
            throw new WhatsAppException('Error retrieving user info: '.$responseData['description']);
        }
     ...
     * @return Response
     */
    private function removeInlineKeyboard($chatId, $messageId): Response
    {
        $parameters = [
            'chat_id' => $chatId,
     ...
     * @return Response
     */
    public function buildServicePayload($message, $matchingMessage, $additionalParameters = []): Response
    {
    ...
    /**
     * Generate the WhatsApp API url for the given endpoint.
     *
     * @param $endpoint
     * @return string
     */
    ...
    /**
     * Generate the WhatsApp File-API url for the given endpoint.
     *
     * @param $endpoint
     * @return string
     */
    ...
    private function postWithExceptionHandling(
        ...
        $responseData['description'] = $responseData['description'] ?? 'No description from WhatsApp';
        $responseData['error_code'] = $responseData['error_code'] ?? 'No error code from WhatsApp';
        $responseData['parameters'] = $responseData['parameters'] ?? 'No parameters from WhatsApp';
